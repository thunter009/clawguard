...
enum Commands {
    ...
    /// Normalize a conventional commit message (stdin or arg)
    NormalizeCommit {
        /// Commit message to normalize (reads stdin if omitted)
...
fn main() {
    ...
    match cli.command {
        ...
        Commands::NormalizeCommit { message } => cmd_normalize_commit(message),
    }
}
...
fn cmd_normalize_commit(message: Option<String>) {
    ...
    let issues = normalizer::validate(&msg);
